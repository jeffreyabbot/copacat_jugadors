...
def parse_fraction(s):
    """Extreu encerts i intents d'un string tipus '2/3' netejant espais."""
    try:
...
def render_boxscore(df, mode="Tradicional"):
    """Renderitza el Boxscore amb el disseny natiu, elegant i interactiu original de Streamlit."""
    st.dataframe(
...
def style_boxscore(df_disp, mode="Tradicional"):
    def apply_row_styles(df):
        # Per defecte, TOTES les columnes s'alineen a la dreta
        css_df = pd.DataFrame(
            "text-align: right;", index=df.index, columns=df.columns
        )

        if "Jugador" in df.columns:
            css_df["Jugador"] = "text-align: left;"
        if "#" in df.columns:
        ...
        for i in df.index:
            ...
            # 4 Factors Individuals
                ...
                if "1. eFG% (Tir)" in df.columns:
                    try:
                        v = float(str(row["1. eFG% (Tir)"]).rstrip("%"))
                        if v >= 60.0:
                ...
                if "2. TOV% (Pèrdues)" in df.columns:
                    try:
                        v = float(str(row["2. TOV% (Pèrdues)"]).rstrip("%"))
                        if v <= 8.0 and v >= 0:  # Molt bona cura de pilota
                            css_df.loc[i, "2. TOV% (Pèrdues)"] = STYLE_RED
                        elif v >= 25.0:  # Excés de pèrdues
                ...
                if "3. OREB% (Rebot)" in df.columns:
                    try:
                        if (
                            float(str(row["3. OREB% (Rebot)"]).rstrip("%"))
                            >= 12.0
                        ):  # Gran rebotador
                ...
                if "4. FT Rate (TL)" in df.columns:
                    try:
                        if (
                            float(str(row["4. FT Rate (TL)"])) >= 0.25
                        ):  # Gran generació TL
                            css_df.loc[i, "4. FT Rate (TL)"] = STYLE_RED
                    except Exception:
                        pass
            # Zones de tir (8/12, 0/3)
            elif mode == "Zones":
                shot_cols = [
                    "Aro (Rim)",
                    "Pintura (Paint)",
            ...
            # Assistències i Punts Generats (MATEIXA INDENTACIÓ QUE ELS ALTRES ELIF)
            elif mode in ["Assistències", "Assistencies"]:
                for pts_col in ["PTS Generats", "Punts Generats"]:
                    if pts_col in df.columns:
                        try:
                            if (
                                float(row[pts_col]) >= 8
                            ):  # Destacat >= 8 pts generats
...
if view_mode == "Partit Individual":
    ...
    # -------------------------------------------------------------
    # 1. PESTAÑA: MÈTRIQUES I 4 FACTORS (DIRECTE D'ADVANCED METRICS)
    # -------------------------------------------------------------
    if active_tab == "📈 Mètriques i 4 Factors":
        ...
        if not g_adv.empty:
            ...
            # 1. LLEGIR ELS 4 FACTORS DIRECTAMENT D'ADVANCED METRICS
            ...
            ft_rate_team = adv_dict_team.get(
                "4. Free Throw Rate (FT Rate)", 0.0
            )
            ft_rate_opp = adv_dict_opp.get("4. Free Throw Rate (FT Rate)", 0.0)

            # 2. TARGETES KPI SUPERIORS
            ...
            # 3. TAULA UNIFICADA DELS 4 FACTORS
            ...
            st.subheader("Desglossament d'Eficiència")
            ...
            with col_table:
                ...
                for _, r in g_adv.iterrows():
                    ...
                    rows.append({"Mètrica": m, "Argentona": tv_str, "Rival": ov_str})
    ...
    # -------------------------------------------------------------
        ...
        if not g_box.empty:
            ...
            if box_mode == "Tradicional":
                ...
                df_def = get_defensive_actions(
                    box_df, pbp_df, selected_game_id
                )
...
# =============================================================
    ...
    if active_tab_season == "📈 Mètriques i 4 Factors":
        ...
        if not comp_lineups.empty or not comp_box.empty:
            ...
            # 1. VOLUMS REALS ACUMULATS (ARGENTONA)
            ...
            # 3. CÀLCUL MATEMÀTIC PONDERAT DELS 4 FACTORS
            efg_team = (
            ...
            # 1. Plays d'Oliver per al TOV% (sense restar rebots)
            ...
            # 2. Possessions de partit i Ritme Pace
            team_poss = float(team_fga + 0.44 * team_fta - team_oreb + team_tov)
            opp_poss = float(opp_fga + 0.44 * opp_fta - opp_oreb + opp_tov)
            norm_poss_tot = 0.5 * (team_poss + opp_poss)
            team_poss_g = (
                norm_poss_tot / num_partits if num_partits > 0 else 0.0
            )
            opp_poss_g = team_poss_g  # Normalitzades iguals per a tots dos

            tot_team_min = comp_box["MIN"].apply(parse_min_to_float).sum()
            if tot_team_min >= 100.0 * num_partits:
                pace_season = (norm_poss_tot / tot_team_min) * 200.0
            else:
                pace_season = team_poss_g
            ...
            # Targetes KPI Superiors: ELS 4 FACTORS DE DEAN OLIVER
            kpi1, kpi2, kpi3, kpi4 = st.columns(4)
            kpi1.metric(
                "1. Tir Efectiu (eFG%)",
                f"{efg_team:.1f}%",
            ...
            # -------------------------------------------------------------
            # QUADRE RESUM: VOLUMS PURS DE LLANÇAMENT (4 FILES NETES)
            # -------------------------------------------------------------
            ...
            with col_v1:
                ...
                st.markdown("##### Volums CB Argentona")
                st.dataframe(
            ...
            # TAULA ÚNICA DELS 4 FACTORS (NOMÉS VALORS PURS)
            ...
            # -------------------------------------------------------------
            # TAULA DE DESGLOSSAMENT D'EFICIÈNCIA I JOC
            # -------------------------------------------------------------
            st.subheader("Desglossament d'Eficiència")
            col_table, _ = st.columns([3, 1])
            ...
            with col_table:
                ...
                # Ràtio AST/TO exclusiu nostre
                ast_to_team = (
                    (team_ast / team_tov) if team_tov > 0 else float(team_ast)
                )

                reb_tot_team = team_oreb + team_dreb
                reb_tot_opp = opp_oreb + opp_dreb
                ...
                table_rows_data = [
                    ...
                    (
                        "Possessions per Partit",
                        f"{team_poss_g:.1f}",
                        f"{opp_poss_g:.1f}",
                        team_poss_g,
                        opp_poss_g,
                        abs(team_poss_g - opp_poss_g) >= 2.0,
                        False,
                    ),
                    (
                        "1. Effective FG% (eFG%)",
                    ...
                    (
                        ...
                        f"{ft_rate_team:.2f} ({team_ftm} TL)",
                        f"{ft_rate_opp:.2f} ({opp_ftm} TL)",
                        ft_rate_team,
                        ft_rate_opp,
                        abs(ft_rate_team - ft_rate_opp) >= 0.05,
                        False,
                    ),
                    (
                    ...
                    (
                        "Rebots Totals per Partit",
                        f"{reb_tot_team/num_partits:.1f} ({team_oreb} Reb.O / {team_dreb} Reb.D)",
                        f"{reb_tot_opp/num_partits:.1f} ({opp_oreb} Reb.O / {opp_dreb} Reb.D)",
                        reb_tot_team / num_partits,
                        reb_tot_opp / num_partits,
                        abs((reb_tot_team - reb_tot_opp) / num_partits) >= 3.0,
                        False,
                    ),
                    (
                        "Assistències per Partit",
                        f"{team_ast/num_partits:.1f} ({team_ast} Tot)",
                        "-",
                        team_ast / num_partits,
                        0.0,
                        False,
                        False,
                    ),
                    (
                        "Ràtio Assistències / Pèrdues",
                        f"{ast_to_team:.2f} AST/PER",
                        "-",
                        ast_to_team,
                        0.0,
                        ast_to_team >= 1.0,
                ...
                # Mètriques de conteig addicionals (Tocs a pintura, Transicions, etc.)
                count_metrics = [
                    "Total Paint Touches",
                    "Total Fastbreaks",
                    "2nd Chance Points",
                ...
                for (
                    ...
                    rows_season.append(
                        {"Mètrica": m, "Argentona": tv_str, "Rivals": ov_str}
                    )
        ...
        if not comp_box.empty and "Name" in comp_box.columns:
            ...
            if box_mode_s == "Tradicional":
                ...
                df_4f_s = get_individual_four_factors(
                    comp_box, comp_lineups, pbp_df, game_id=None
                )
                ...
                df_def_s = get_defensive_actions(comp_box, pbp_df, game_id=None)
                ...
                render_boxscore(df_sorted, "Context")
                # 👇 XARXA D'ASSISTÈNCIES ACUMULADA DE LA TEMPORADA
