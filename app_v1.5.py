    ...
    def apply_row_styles(df):
        ...
        for i in df.index:
            ...
            # Zones de tir
                ...
                # Tirs de 2 a la pintura i mitja distància
                ...
                # Triples (A partir del 50% ja és excel·lent -> Vermell)
...
# -------------------------------------------------------------
    ...
    if active_tab == "📈 Mètriques i 4 Factors":
        ...
        if not g_adv.empty:
            ...
            # 7. TAULA DE DESGLOSSAMENT D'EFICIÈNCIA (SENSE 4 FACTORS)
            st.subheader("📊 Desglossament d'Eficiència de Tir i Joc")
            col_table, _ = st.columns([3, 1])

            with col_table:
                # PPS PUR DE TIR EN JOC (Punts de Camp / FGA)
                pps_team_real = (
                ...
                # Mètriques de conteig del report d'Advanced Metrics
                for _, r in g_adv.iterrows():
                    m_raw = str(r["Metric"]).strip()
                    m_lower = m_raw.lower()

                    # OMETRE ELS 4 FACTORS I PPS VELL
                    if any(
                        term in m_lower
                        for term in [
                            "efg",
                    ...
                    m_nom = METRIC_NAMES_CAT.get(m_raw, m_raw)
                    tv_val = (
                        float(r["Team_Value"])
                    ...
                    is_out = abs(tv_val - ov_val) >= 2.0
                    low_better = "pèrdua" in m_nom.lower() or "lto" in m_nom.lower() or "dto" in m_nom.lower()

                    table_rows_single.append(
                        (
                            m_nom,
                            f"{int(tv_val)}"
        ...
        if not g_box.empty:
            box_mode = st.segmented_control(
                ...
                options=[
                    ...
                    "4 Factors Individuals",
                    "Accions Defensives",
                    "Impacte i Context",  # <-- Nom actualitzat
...
# --- MODO 2: TOTALS ACUMULATS DE LA TEMPORADA ---
    ...
    if active_tab_season == "📈 Mètriques i 4 Factors":
        if not comp_lineups.empty or not comp_box.empty:
            ...
            # 8. TAULA DE DESGLOSSAMENT D'EFICIÈNCIA
            st.subheader("📊 Desglossament d'Eficiència de Tir i Joc")
            col_table, _ = st.columns([3, 1])
            ...
            with col_table:
                ...
                for cm in count_metrics:
                    if cm in adv_sums_dict_t:
                        ...
                        c_nom = METRIC_NAMES_CAT.get(cm, cm)
                        table_rows_season.append(
                            (
                                c_nom,
    ...
    # -------------------------------------------------------------
    # -------------------------------------------------------------
    # 2. SECCIÓ BOX SCORE (TOTALS ACUMULATS AMB FILTRE DE MINUTS)
    # -------------------------------------------------------------
    elif active_tab_season == "📋 Box Score":
        if not comp_box.empty and "Name" in comp_box.columns:
            # 1. Càlcul de minuts totals de cada jugador a la temporada
            ...
            # 1. Filtre de Minuts a sobre (alineat a l'esquerra)
            col_b_filt, col_sp = st.columns([1, 2])
            with col_b_filt:
                min_m_filt = st.slider(
                    "Minuts mínims (Temporada)",
                    min_value=0,
            ...
            # 2. Selector de pestanyes a sota ocupant el 100% de l'amplada (sense tallar cap text)
            ...
            # 3. Filtrar només jugadors que superin els minuts mínims seleccionats
            ...
            if box_mode_s == "Tradicional":
                ...
                # PPP net a 1 sol decimal
                df_trad_s["PPP"] = (p_agg["PTS"] / p_agg["PJ"]).apply(
                    lambda x: f"{x:.1f}"
                )

                # Fraccions netes sense decimals (ex: 8/14)
                df_trad_s["T2 (A/I)"] = (
                    p_agg["T2M"].astype(int).astype(str)
                    + "/"
                    + p_agg["T2A"].astype(int).astype(str)
                )
                df_trad_s["%T2"] = (
                ...
                df_trad_s["T3 (A/I)"] = (
                    p_agg["T3M"].astype(int).astype(str)
                    + "/"
                    + p_agg["T3A"].astype(int).astype(str)
                )
                df_trad_s["%T3"] = (
                ...
                df_trad_s["TL (A/I)"] = (
                    p_agg["FTM"].astype(int).astype(str)
                    + "/"
                    + p_agg["FTA"].astype(int).astype(str)
                )
                df_trad_s["%TL"] = (
                ...
                # Ordenació intel·ligent per DORSAL NUMÈRIC (7, 8, 9, 11, 13...)
            ...
            elif box_mode_s == "Xarxa de Passades (Assistències)":
