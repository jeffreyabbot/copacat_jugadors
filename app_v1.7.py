    ...
    def apply_row_styles(df):
        ...
        for i in df.index:
            ...
            if mode == "Tradicional":
                ...
                if "REB_T" in df.columns and to_f(row["REB_T"]) >= 6:
                    css_df.loc[i, "REB_T"] = STYLE_RED
                if "AST" in df.columns and to_f(row["AST"]) >= 4:
                    css_df.loc[i, "AST"] = STYLE_RED
                if "REC" in df.columns and to_f(row["REC"]) >= 3:
                    css_df.loc[i, "REC"] = STYLE_RED
                if "PER" in df.columns and to_f(row["PER"]) >= 3:
            ...
            # 4. Impacte i Context
                ...
                for per_col in ["PER Totals", "Pèrdues"]:
                    if per_col in df.columns and to_f(row[per_col]) >= 3:
                        css_df.loc[i, per_col] = STYLE_BLUE

                if "Vives (LTO)" in df.columns and to_f(row["Vives (LTO)"]) >= 2:
                    css_df.loc[i, "Vives (LTO)"] = STYLE_BLUE
                if "Faltes Com." in df.columns and to_f(row["Faltes Com."]) >= 4:
                    css_df.loc[i, "Faltes Com."] = STYLE_BLUE
                if "Faltes Reb." in df.columns and to_f(row["Faltes Reb."]) >= 4:
                    css_df.loc[i, "Faltes Reb."] = STYLE_RED

            # 5. Accions Defensives
            elif mode == "Defensa":
                if "Defleccions" in df.columns and to_f(row["Defleccions"]) >= 3:
                    css_df.loc[i, "Defleccions"] = STYLE_RED
                if "Robatoris (REC)" in df.columns and to_f(row["Robatoris (REC)"]) >= 2:
                    css_df.loc[i, "Robatoris (REC)"] = STYLE_RED
                if "Taps Totals (TAP)" in df.columns and to_f(row["Taps Totals (TAP)"]) >= 2:
                    css_df.loc[i, "Taps Totals (TAP)"] = STYLE_RED
                if "Impacte Defensiu (Total)" in df.columns and to_f(row["Impacte Defensiu (Total)"]) >= 5:
                    css_df.loc[i, "Impacte Defensiu (Total)"] = STYLE_RED

            # 6. Assistències
            elif "assist" in str(mode).lower():
                for pts_col in ["PTS Generats", "Punts Generats"]:
                    if pts_col in df.columns and to_f(row[pts_col]) >= 8:
            ...
            # 7. Rebot i Segona Oportunitat
                ...
                if "REB/Min" in df.columns:
                    ...
                    elif 0 <= v <= 0.12 and to_f(row.get("REB_T", 0)) <= 3:
                        css_df.loc[i, "REB/Min"] = STYLE_BLUE
                if "REB_T" in df.columns and to_f(row["REB_T"]) >= 6:
                    css_df.loc[i, "REB_T"] = STYLE_RED
                if "REB_O" in df.columns and to_f(row["REB_O"]) >= 3:
                    css_df.loc[i, "REB_O"] = STYLE_RED
                if "Pts 2a Op." in df.columns and to_f(row["Pts 2a Op."]) >= 4:
                    css_df.loc[i, "Pts 2a Op."] = STYLE_RED
                if "Fallades 2a Op." in df.columns and to_f(row["Fallades 2a Op."]) >= 2:
